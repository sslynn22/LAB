...
I     = zeros(T*packet, 3, 30);   % 정규화된 허수부(imaginary value) 저장 배열 
R     = zeros(T*packet, 3, 30);   % 정규화된 실수부(real value) 저장 배열
...
% ==============================================================================================================
% 데이터 로드 및 AP별 처리
% : AP별로 문자 식별자(예: 'A', 'B', ... 'I')를 이용해 각 AP에 해당하는 CSI 데이터, 패킷 수(count) 텍스트, 그리고 RSSI 데이터를 로드
% ==============================================================================================================
...
for x = 0:0
    for y = 0:0
        ...
        for z = 1:9 % number of AP
            ...
            % load(s);로 불러온 .mat 파일에는 보통 csi_matrices라는 4차원 배열이 들어 있다.
            % csi_matrices(packet 수, 안테나 수, 서브캐리어 수, 기타 차원)
            csi_matrice = squeeze(csi_matrices(:, 1, :, :));
            % 마지막 측정 포인트가 시작하는 지점까지의 전체 패킷을 사용하겠다는 뜻.
            % 예. c(141) = 14100이라면 → 1:14100 → 총 14100개의 CSI 데이터를 자르겠다는 뜻
            % csi_matrices에는 전체 측정 포인트(141개)보다 더 많은 패킷이 포함되어 있을 수도 있어서 → 측정 포인트 수만큼 정확히 잘라서 사용해야 하기 때문



            % ====================================================================
            % 각 포인트별로 CSI 및 RSSI 데이터를 100패킷 단위로 정리 -> 누락된 패킷을 채우기 위함
            % : 모든 포인트가 정확히 100개의 패킷을 가지도록 데이터의 길이 맞춤
            % ====================================================================
            ...
            % =================================================================
            % CSI 데이터의 진폭 및 위상 처리
            % : ab는 CSI의 진폭 정보를 나타내며, 
            %   원래는 RSSI를 기반으로 보정 계수를 곱해 정규화하려는 의도였으나, 현재는 주석 처리
            % =================================================================
            ...
            % =========================================
            % 안테나 간 CSI 비율 및 정규화
            % : 실수부와 허수부를 각각 정규화한 후 결합하여, 
            %   AP 간 상호위상 정보를 담은 3차원 배열(RI)을 생성
            % =========================================
            ...
            % 위상 차이를 계산 및 보정 -> csi 데이터로 사용하지는 않음. 
